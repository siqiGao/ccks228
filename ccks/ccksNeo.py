...
def get_id_by_name(name):
    graph1 = Graph("http://10.1.1.30:9700/browser/", username="neo4j", password="123456")
    statement = "match (n:Instance)-[r]->(m:Instance) where n.name = \"" + name + "\" return id(n)"
...
def get_entity_info_by_neoid(neoid):
    graph1 = Graph("http://10.1.1.30:9700/browser/", username="neo4j", password="123456")
    statement = "match (n:Instance)-[r]->(m:Instance) where id(n) = " + str(neoid) + " return r.value, m.name, id(m)"
    # statement = "match (n:Instance) where n.name= \""+"红楼梦"+"\" return n"
    list1 = graph1.run(statement).data()
    #print(list1)
    result = []
...
def get_related_entities_by_neoid(neoid):####
    statement = "match (n:Instance)-[r]->(m:Instance) where id(n) = " + str(neoid) + " return r.value,m.name,id(m)"
    related_entities = []
    print(statement)
    result = execute(statement)
...
def lalala(id):
    print(1)
    jieguo = get_entity_info_by_id(id)
    return jieguo
